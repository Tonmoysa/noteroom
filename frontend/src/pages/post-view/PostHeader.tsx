    ...
    return (
        <div className="post-header">
            <div className="post-header__info">
                ...
                <img className="post-author-pic" src={noteData?.owner?.profile_pic} alt="Author Picture" />
                <span className="post-author-name" onClick={() => navigate(`/user/${noteData?.owner?.username}`)}>{noteData?.owner?.displayname}</span>
            </div>

            <div className="post-header__actions">
                { !noteData?.isPostOwner && <button className="request-btn db-note-card-request-option" onClick={() => setShowReqModal(prev => !prev)}>Request</button> }
                <button className={"save-btn " + (isSaved ? "saved" : "")} onClick={() => saveNote({ postID: noteData?.postID!, title: noteData?.title!, content: noteData?.content!}, isSaved!)}>
            ...
            <RequestModal modalShow={[showReqModal, setShowReqModal]} recipientData={{profile_pic: noteData?.owner?.profile_pic, displayname: noteData?.owner?.displayname, username: noteData?.owner?.username }}></RequestModal>
        </div>
    )
}